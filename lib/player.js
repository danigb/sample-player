...
function SamplePlayer (ac, source, options) {
  ...
  player.emit = function (event, when, obj, opts) {
    var onEvent = player.onevent
    if (typeof onEvent === 'function') onEvent(event, when, obj, opts)
    else if (Array.isArray(onEvent)) onEvent.forEach((fn) => { fn(event, when, obj, opts) })

    var callback = player['on' + event]
    if (typeof callback === 'function') callback(when, obj, opts)
    else if (Array.isArray(callback)) callback.forEach((fn) => { fn(when, obj, opts) })
  }
